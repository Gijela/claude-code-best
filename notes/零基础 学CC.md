...
**这就是"调度系统"的核心**：不是简单的"有工具调用就继续"，而是包含多种恢复/重试路径的状态机。

---

### QueryEngine：会话编排器

`QueryEngine` 在单轮 Agentic Loop 之上管理多轮会话：
...
核心方法 `submitMessage()` 是 `async *Generator`，逐步 yield 消息让 REPL 实时展示进度。
...
| **文件历史快照** | `src/utils/fileHistory.ts` |
